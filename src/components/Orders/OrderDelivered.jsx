...
export default function UsersTable() {
  ...
  const exportToExcel = () => {
    ...
    downloadBlob(csv, "delivered_orders_export.csv", "text/csv;charset=utf-8;");
    setIsExportMenuOpen(false);
  };
  ...
  const exportToDoc = () => {
    const html = getExportHtml("Delivered Orders Export");
    if (!html) {
      setIsExportMenuOpen(false);
      return;
    }
    downloadBlob(html, "delivered_orders_export.doc", "application/msword");
    setIsExportMenuOpen(false);
  };
            ...
            {isError && !isLoading && (
              <tr>
                <td colSpan="9" className="text-center p-6 text-red-500 font-semibold">
                  Failed to load delivered orders. Please try again.
                </td>
              </tr>
            )}
